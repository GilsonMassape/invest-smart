...
const getSafePreferences = (preferences: Preferences): Preferences => ({
  ...preferences,
...
const calculateTotalPortfolioMarketValue = (
  ...
  for (const asset of assets) {
    const position = positionsByTicker.get(normalizeTicker(asset.ticker))
...
const buildRankedAsset = (
  ...
  return {
    ...
    safeCurrentValue: snapshot.currentMarketValue,
...
const sortByFinalScoreDesc = (
  ...
  const scoreDiff = right.score.finalScore - left.score.finalScore
...
export const buildRanking = (
  ...
  const safeAssets = assets.filter(
    (asset) => asset?.ticker && asset.ticker.trim().length > 0
  )
  ...
  const safePreferences = getSafePreferences(preferences)

  const portfolioContext = buildPortfolioContext(safeAssets, safePositions)

  const rankedAssets = safeAssets.map((asset) =>
    buildRankedAsset(
      asset,
      safePreferences,
      portfolioContext.positionsByTicker.get(normalizeTicker(asset.ticker)),
  ...
  const rankedWithPercentile: RankedAssetWithPercentile[] = rankedAssets.map(
    (asset, index) => ({
      ...asset,
